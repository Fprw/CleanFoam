...
report_type = st.selectbox("اختر نوع التقرير", ["تقرير يومي", "تقرير مخصص", "تقرير المصروفات", "تقرير العمال", "مقارنة بين فترتين"])

if report_type == "تقرير يومي":
    date = st.date_input("اختر التاريخ")
    df = sales[sales['date'] == str(date)]
    st.write(f"تقرير المبيعات ليوم {date}")
    st.dataframe(df)
    ...
    st.write(f"{start1} إلى {end1}")
    st.metric("إجمالي الكاش", df1[df1["payment_type"] == "كاش"]["amount"].sum())
    st.metric("إجمالي الشبكة", df1[df1["payment_type"] == "شبكة"]["amount"].sum())

    st.subheader("الفترة 2")
    st.write(f"{start2} إلى {end2}")
    st.metric("إجمالي الكاش", df2[df2["payment_type"] == "كاش"]["amount"].sum())
    st.metric("إجمالي الشبكة", df2[df2["payment_type"] == "شبكة"]["amount"].sum())
